<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
      ...
      <UButton icon="i-lucide-plus" :disabled="activeCompanyOptions.length === 0" @click="openCreate"> New journal </UButton>
    </div>
    ...
    <UCard>
      <DataTable v-model:sort="sort" :rows="pagedRows" :columns="columns" :loading="loading" refreshable numbered @refresh="load">
        ...
        <template #empty-state>
          ...
          <EmptyState v-else icon="i-lucide-book-text" title="No journals yet" description="Create the first journal to get started.">
            <template #action>
              <UButton :disabled="activeCompanyOptions.length === 0" icon="i-lucide-plus" @click="openCreate">New journal</UButton>
            </template>
          </EmptyState>
        </template>
      </DataTable>
    ...
    <UModal v-model:open="showCreate" title="New journal">
      <template #body>
        <DynamicForm
          v-model="createForm"
          :fields="formFields"
    ...
    <UModal v-model:open="showEdit" :title="`Edit journal '${editingJournal?.name ?? ''}'`">
      <template #body>
        <DynamicForm
          v-model="editForm"
          :fields="formFields"
    ...
    <ConfirmModal
      ...
      :description="`Delete journal '${confirmDelete?.name ?? ''}'? This cannot be undone.`"
...
import type { Journal, JournalPayload } from '~/composables/useJournals'
...
const formFields = computed<FieldDef[]>(() => [
  ...
  { name: 'code', required: true, hint: 'e.g. GEN, SALES.' },
  { name: 'name', required: true, hint: 'e.g. General Journal.' },
...
const {
  ...
  confirmDelete,
  onDelete
} = useCrudModals<Journal, JournalPayload>(
  {
