<template>
  <div>
    ...
    <PageHeader
      ...
      :crumbs="[{ label: 'Reports', to: '/reports' }, { label: 'Sales reports' }, { label: 'Pending orders' }]"
    />
...
import type { SalesPendingOrderRow, SalesPendingOrders } from '~/composables/useSalesReports'

definePageMeta({ middleware: 'admin' })

const { companyId, dateFrom, dateTo, activeCompanyOptions, ensureLoaded } = useReportFilters()
const { pendingOrders: fetchPendingOrders } = useSalesReports()

const loading = ref(false)
const error = ref('')
const pendingOrders = ref<SalesPendingOrders | null>(null)

const columns: ColumnDef<SalesPendingOrderRow>[] = [
