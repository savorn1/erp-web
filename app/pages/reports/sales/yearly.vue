<template>
  <div>
    ...
    <PageHeader
      title="Yearly sales"
      description="Order count and revenue grouped by year."
      :crumbs="[{ label: 'Reports', to: '/reports' }, { label: 'Sales reports' }, { label: 'Yearly sales' }]"
    />
    ...
    <UCard v-else-if="yearly">
      <DataTable :rows="yearly.rows" :columns="columns" exportable export-filename="sales-yearly">
...
const { yearly: fetchYearly } = useSalesReports()

const loading = ref(false)
const error = ref('')
const yearly = ref<SalesByPeriod | null>(null)

const columns = computed<ColumnDef<SalesByPeriodRow>[]>(() => [
  { key: 'period', label: 'Year', footer: () => 'Total' },
  { key: 'orderCount', label: 'Orders' },
  { key: 'revenue', label: 'Revenue', type: 'currency', footer: () => yearly.value?.totalRevenue }
])
