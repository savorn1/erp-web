<template>
  <div>
    ...
    <PageHeader
      ...
      description="Order count and spend grouped by year."
      :crumbs="[{ label: 'Reports', to: '/reports' }, { label: 'Purchase reports' }, { label: 'Yearly purchases' }]"
    />
    ...
    <UCard v-else-if="yearly">
      <DataTable :rows="yearly.rows" :columns="columns" exportable export-filename="purchase-yearly">
...
const { yearly: fetchYearly } = usePurchaseReports()

const loading = ref(false)
const error = ref('')
const yearly = ref<PurchaseByPeriod | null>(null)

const columns = computed<ColumnDef<PurchaseByPeriodRow>[]>(() => [
  { key: 'period', label: 'Year', footer: () => 'Total' },
  { key: 'orderCount', label: 'Orders' },
  { key: 'amount', label: 'Amount', type: 'currency', footer: () => yearly.value?.totalAmount }
])
...
async function load() {
  ...
  error.value = ''
  try {
    yearly.value = await fetchYearly({
