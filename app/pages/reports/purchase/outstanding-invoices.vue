<template>
  <div>
    ...
    <PageHeader
      ...
      :crumbs="[{ label: 'Reports', to: '/reports' }, { label: 'Purchase reports' }, { label: 'Outstanding invoices' }]"
    />
    ...
    <template v-else-if="outstandingInvoices">
      ...
      <UCard>
        <DataTable :rows="outstandingInvoices.rows" :columns="columns" exportable export-filename="purchase-outstanding-invoices">
...
import type { PurchaseOutstandingInvoiceRow, PurchaseOutstandingInvoices } from '~/composables/usePurchaseReports'

definePageMeta({ middleware: 'admin' })

const { companyId, dateFrom, dateTo, activeCompanyOptions, ensureLoaded } = useReportFilters()
const { outstandingInvoices: fetchOutstandingInvoices } = usePurchaseReports()

const loading = ref(false)
const error = ref('')
const outstandingInvoices = ref<PurchaseOutstandingInvoices | null>(null)

const columns: ColumnDef<PurchaseOutstandingInvoiceRow>[] = [
