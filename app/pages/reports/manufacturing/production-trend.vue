<template>
  <div>
    ...
    <PageHeader
      ...
      :crumbs="[{ label: 'Reports', to: '/reports' }, { label: 'Manufacturing reports' }, { label: 'Production trend' }]"
    />
    ...
    <UCard v-else-if="report">
      <DataTable :rows="report.rows" :columns="columns" exportable export-filename="manufacturing-production-trend">
...
import type { ProductionTrend, ProductionTrendRow } from '~/composables/useManufacturingReports'
...
const { productionTrend: fetchProductionTrend } = useManufacturingReports()

const loading = ref(false)
const error = ref('')
const report = ref<ProductionTrend | null>(null)

const columns: ColumnDef<ProductionTrendRow>[] = [
...
async function load() {
  ...
  error.value = ''
  try {
    report.value = await fetchProductionTrend({
