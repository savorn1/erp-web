<template>
  <div>
    ...
    <PageHeader
      ...
      :crumbs="[{ label: 'Reports', to: '/reports' }, { label: 'Accounts payable reports' }, { label: 'AP summary' }]"
    />
    ...
    <div v-else-if="summary" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <StatTile label="Total outstanding" :value="formatCurrency(summary.totalOutstanding)" icon="i-lucide-landmark" color="primary" />
      ...
      <StatTile
        ...
        :sublabel="`${summary.overdueSupplierCount} overdue`"
        icon="i-lucide-truck"
        color="info"
      />
      <StatTile
...
import type { ApSummary } from '~/composables/useApReports'

definePageMeta({ middleware: 'admin' })

const { companyId, asOfDate, activeCompanyOptions, ensureLoaded } = useReportFilters()
const { summary: fetchSummary } = useApReports()

const loading = ref(false)
const error = ref('')
const summary = ref<ApSummary | null>(null)
