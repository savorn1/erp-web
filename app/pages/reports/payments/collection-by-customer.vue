<template>
  <div>
    ...
    <PageHeader
      ...
      description="Received, refunded, and net collected per customer for a period."
      :crumbs="[{ label: 'Reports', to: '/reports' }, { label: 'Payment reports' }, { label: 'Collection by customer' }]"
    />
...
import type { CollectionByCustomer, CollectionByCustomerRow } from '~/composables/usePaymentReports'

definePageMeta({ middleware: 'admin' })

const { companyId, dateFrom, dateTo, activeCompanyOptions, ensureLoaded } = useReportFilters()
const { collectionByCustomer: fetchCollectionByCustomer } = usePaymentReports()

const loading = ref(false)
const error = ref('')
const report = ref<CollectionByCustomer | null>(null)

const columns: ColumnDef<CollectionByCustomerRow>[] = [
...
async function load() {
  ...
  error.value = ''
  try {
    report.value = await fetchCollectionByCustomer({
