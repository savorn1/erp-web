<template>
  <div>
    ...
    <UCard v-else>
      <DataTable :rows="rows" :columns="columns" exportable export-filename="inventory-stock-by-customer">
...
async function load() {
  ...
  try {
    const [res, productsRes] = await Promise.all([
      stockByCustomer({ companyId: companyId.value, warehouseId: warehouseId.value }),
