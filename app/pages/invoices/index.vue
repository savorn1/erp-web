    ...
    <UModal v-model:open="showView" :title="`Invoice — ${viewingInvoice?.invoiceNumber ?? ''}`" :ui="{ content: 'sm:max-w-3xl' }">
      <template #body>
        ...
        <template v-else-if="viewingInvoice">
          ...
          <div v-if="viewingInvoice.foreignCurrency" class="flex justify-end text-xs text-gray-500 dark:text-gray-400 mb-1">
...
function invoiceWorkflowHint(status: InvoiceStatus) {
  if (status === 'DRAFT') return 'Next: approve to post the customer charge'
