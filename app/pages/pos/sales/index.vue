      ...
      <DataTable :rows="pagedRows" :columns="columns" :loading="loading" refreshable numbered @refresh="load">
        <template #actions-data="{ row }">
          <div class="flex items-center gap-2">
            ...
            <UButton v-if="row.status === 'COMPLETED'" size="xs" color="error" variant="soft" icon="i-lucide-undo-2" @click="confirmVoid = row">
              Void
            </UButton>
          </div>
        </template>
      ...
      <template #body>
        <div v-if="viewing" class="space-y-3 text-sm">
          <dl class="grid grid-cols-2 gap-2">
            ...
            <div><dt class="text-gray-500 dark:text-gray-400">Customer</dt><dd>{{ viewing.customerName ?? '—' }}</dd></div>
            <div><dt class="text-gray-500 dark:text-gray-400">Date</dt><dd>{{ formatDateTime(viewing.saleDate) }}</dd></div>
            <div><dt class="text-gray-500 dark:text-gray-400">Status</dt><dd>{{ viewing.status }}</dd></div>
          </dl>
          ...
          <dl class="space-y-1 pt-2 border-t border-gray-200 dark:border-gray-800">
            <div class="flex justify-between"><dt>Subtotal</dt><dd>{{ formatCurrency(viewing.subtotal) }}</dd></div>
            <div class="flex justify-between"><dt>Discount</dt><dd>-{{ formatCurrency(viewing.discountAmount) }}</dd></div>
            <div class="flex justify-between"><dt>Tax</dt><dd>{{ formatCurrency(viewing.taxAmount) }}</dd></div>
            <div class="flex justify-between font-semibold"><dt>Total</dt><dd>{{ formatCurrency(viewing.totalAmount) }}</dd></div>
          </dl>
