<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Cost centers</h1>
      <UButton icon="i-lucide-plus" :disabled="activeCompanyOptions.length === 0" @click="openCreate"> New cost center </UButton>
    </div>
      ...
      <DataTable v-model:sort="sort" :rows="pagedRows" :columns="columns" :loading="loading" refreshable numbered @refresh="load">
        ...
        <template #empty-state>
          <EmptyState
            ...
            title="No cost centers match your filters"
          ...
          <EmptyState v-else icon="i-lucide-building-2" title="No cost centers yet" description="Create the first cost center to get started.">
            <template #action>
              <UButton :disabled="activeCompanyOptions.length === 0" icon="i-lucide-plus" @click="openCreate">New cost center</UButton>
            </template>
          </EmptyState>
        </template>
      </DataTable>
    ...
    <ConfirmModal
      ...
      :description="`Delete cost center '${confirmDelete?.name ?? ''}'? This cannot be undone.`"
...
import type { CostCenter, CostCenterPayload } from '~/composables/useCostCenters'

definePageMeta({ middleware: 'admin' })

const { list, create, update, remove } = useCostCenters()
const { list: listCompanies } = useCompanies()

const rows = ref<CostCenter[]>([])
...
const formFields = computed<FieldDef[]>(() => [
  ...
  { name: 'code', required: true, hint: 'e.g. CC-SALES.' },
  { name: 'name', required: true, hint: 'e.g. Sales Department.' },
...
const {
  ...
  confirmDelete,
  onDelete
} = useCrudModals<CostCenter, CostCenterPayload>(
  {
