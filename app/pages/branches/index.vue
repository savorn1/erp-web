<template>
  <div>
    ...
    <UModal v-model:open="showCreate" title="New branch" :ui="{ content: 'sm:max-w-2xl' }">
      <template #body>
        <DynamicForm
          v-model="createForm"
          :fields="branchFields"
    ...
    <UModal v-model:open="showEdit" :title="`Edit branch '${editingBranch?.name ?? ''}'`" :ui="{ content: 'sm:max-w-2xl' }">
      <template #body>
        <DynamicForm
          v-model="editForm"
          :fields="branchFields"
    ...
    <ConfirmModal
      ...
      title="Delete branch"
      :description="`Delete branch '${confirmDelete?.name ?? ''}'? This cannot be undone.`"
...
import type { Branch, BranchPayload } from '~/composables/useBranches'
...
const {
  ...
  confirmDelete,
  onDelete
} = useCrudModals<Branch, BranchPayload>(
  {
...
async function toggleStatus(row: Branch) {
  ...
  try {
    ...
    toast.add({ title: row.active ? 'Branch deactivated' : 'Branch activated', color: 'success' })
    await load()
  } catch (err) {
