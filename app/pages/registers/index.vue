<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Registers</h1>
      <UButton icon="i-lucide-plus" :disabled="activeCompanyOptions.length === 0" @click="openCreate">New register</UButton>
    </div>
    ...
    <UCard class="mb-4">
      <div class="flex flex-wrap gap-3">
        <UInput v-model="search" placeholder="Search code" icon="i-lucide-search" class="w-56" />
      ...
      <DataTable :rows="pagedRows" :columns="columns" :loading="loading" refreshable numbered @refresh="load">
        <template #actions-data="{ row }">
          <div class="flex items-center gap-2">
            ...
            <UButton size="xs" color="error" variant="soft" icon="i-lucide-trash-2" :disabled="row.hasOpenSession" @click="confirmDelete = row">
              Delete
            </UButton>
          </div>
        </template>
        <template #empty-state>
          <EmptyState
            ...
            title="No registers match your filters"
          ...
          <EmptyState v-else icon="i-lucide-monitor" title="No registers yet" description="Create the first register to get started.">
            <template #action>
              <UButton :disabled="activeCompanyOptions.length === 0" icon="i-lucide-plus" @click="openCreate">New register</UButton>
            </template>
          </EmptyState>
        </template>
      </DataTable>
    ...
    <ConfirmModal
      ...
      :description="`Delete register '${confirmDelete?.name ?? ''}'? This cannot be undone.`"
...
import type { Register, RegisterPayload } from '~/composables/useRegisters'

definePageMeta({ middleware: 'admin' })

const { list, create, update, remove } = useRegisters()
const { list: listCompanies } = useCompanies()
const { list: listWarehouses } = useWarehouses()

const rows = ref<Register[]>([])
...
const { page, pageSize, total, rows: pagedRows, search } = useClientTable(rows, { pageSize: 10, searchFields: ['code', 'name'] })
...
const columns: ColumnDef<Register>[] = [
  ...
  { key: 'hasOpenSession', label: 'Session', type: 'boolean', trueLabel: 'Open', trueColor: 'warning', falseLabel: 'Closed', falseColor: 'neutral' },
  { key: 'actions', label: '' }
]
...
const {
  ...
  confirmDelete,
  onDelete
} = useCrudModals<Register, RegisterPayload>(
  {
  ...
  load,
  {
    ...
    toForm: (row) => ({ companyId: row.companyId, warehouseId: row.warehouseId, code: row.code, name: row.name, active: row.active }),
    toPayload: (values) => ({
