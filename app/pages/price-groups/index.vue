<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
      ...
      <UButton icon="i-lucide-plus" @click="openCreate"> New price group </UButton>
    </div>
...
const {
  ...
  load,
  {
    ...
    toForm: (row) => ({ name: row.name, discountPercent: row.discountPercent ?? undefined, active: row.active }),
    toPayload: (values) => ({
      name: values.name,
