  ...
  <div>
    ...
    <UCard>
      <DataTable
        ...
        numbered
        exportable
        export-filename="product-brands"
    ...
    <UModal v-model:open="showCreate" title="New brand">
      <template #body>
        <DynamicForm
          v-model="createForm"
          :fields="formFields"
    ...
    <UModal v-model:open="showEdit" :title="`Edit brand '${editingBrand?.name ?? ''}'`">
      <template #body>
        <DynamicForm
          v-model="editForm"
          :fields="formFields"
...
import type { ProductBrand, ProductBrandPayload } from '~/composables/useProductBrands'
...
const {
  ...
  showEdit, editing, editError, editingRow: editingBrand, editForm, openEdit, onEdit,
  deleting, confirmDelete, onDelete
} = useCrudModals<ProductBrand, ProductBrandPayload>(
  {
